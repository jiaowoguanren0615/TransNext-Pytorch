...
class AggregatedAttention(nn.Module):
    ...
    def forward(self, x, H, W, relative_pos_index, relative_coords_table):
        ...
        attn_local = ((q_norm_scaled.unsqueeze(-2) @ k_local).squeeze(-2) + self.relative_pos_bias_local.unsqueeze(1)).masked_fill(self.padding_mask, float('-inf'))
...
@register_model
def transnext_micro(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
...
@register_model
def transnext_tiny(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model = TransNeXt(window_size=[3, 3, 3, None],
...
@register_model
def transnext_small(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model = TransNeXt(window_size=[3, 3, 3, None],
                      patch_size=4, embed_dims=[72, 144, 288, 576], num_heads=[3, 6, 12, 24],
                      mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
@register_model
def transnext_base(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model = TransNeXt(window_size=[3, 3, 3, None],
...
#     net = transnext_base(num_classes=5)
